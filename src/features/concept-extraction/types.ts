...
export type ExtractedConcept = {
  ...
  name: string;
  /** Plain-language explanation that gives downstream persistence enough context to place the concept. */
  description: string;
};
 ...
 * @description Lets callers preserve quizlet-style term-definition structure instead of treating all pasted text as prose.
 */
...
/**
 * Current generated metaphor attached to a stored concept.
...
export type RoomSummary = {
  ...
  name: string;
  /** User-scoped slug used for deterministic matching and future routing. */
  slug: string;
...
/**
 * Payload accepted by the concept persistence API.
 * @description Accepts already-extracted concepts so storage can remain separate from raw-input extraction.
 */
export type PersistConceptsInput = {
  /** Concepts extracted from text, PDFs, or URLs and ready for persistence. */
  concepts: ExtractedConcept[];
};

/**
 * Result returned after concepts are persisted.
